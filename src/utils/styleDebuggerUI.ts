// StyleDebuggerUI.ts
type LogMode = 'instant' | 'queued';

export interface Breakpoint {
    key: string;
    condition: (value: string | null) => boolean;
}

export class StyleDebuggerUI {
    private topPanel: HTMLDivElement;
    private logPanel: HTMLDivElement;
    private logTabs: HTMLDivElement;
    private itemPanel: HTMLDivElement;
    private timelinePanel: HTMLDivElement;
    private currentTab: 'item' | 'timeline' = 'timeline';
...
if (window.monitor && originalPause) {
    window.monitor.pause = function(...args: any[]) {
        ...
        this.logTabs.addEventListener('click', e => {
            ...
            const tab = btn.dataset.tab as 'item' | 'timeline';
            if (!tab) return;
            this.currentTab = tab;
        ...
        // グローバルログ関数
        window['dbgLog'] = (msg: string) => this.externalLog(msg);
    }

    // 現在値パネル更新
    public displayChange(key: string, value: string | null, changed: boolean) {
        let row = this.topPanel.querySelector<HTMLDivElement>(`.row[data-key="${key}"]`);
        if (!row) {
            ...
            const valueSpan = row.querySelector<HTMLSpanElement>('.value')!;
    ...
    // ログパネル更新
    public logChange(key: string, value: string | null, logMode: LogMode) {
        // ブレイクポイントチェック
        ...
        if (logMode === 'instant') {
            ...
            this.logTimer = window.setTimeout(() => {
                if (this.pendingLog) this.appendItemLog(key, this.pendingLog);
...
private appendItemLog(key: string, msg: string) {
    // keyごとの div を取得または作成
    let panel = this.itemPanel.querySelector<HTMLDivElement>(`.item-log[data-key="${key}"]`);
    if (!panel) {
    ...
    const container = panel.querySelector<HTMLDivElement>('.item-container')!;
...
private appendLog(msg: string) {
    ...
    // ブレイクポイント追加
    public addBreakpoint(bp: Breakpoint) {
